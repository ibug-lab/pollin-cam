...
This will configure the web interface and provide the URL to access the Witty Pi device and enter the scheduling information. When you first open the web UI, it will show an error that the witty pi software can't be found. This is because the configuration file has the incorrect username specified for our pi. To fix this, we'll open up that file, and replace "pi" in the file paths with "ibuglab". Run the following line, and then replace pi in the filepath with ibuglab in the 5 last lines of that configuration file. 
